...
namespace HyperActive.Dominator
{
	...
	public class DomTester : IDisposable
	{
		...
		public ITypeTester Type(string typeName)
		{
			Type type = _compiled.GetType(typeName);
			if (type == null)
				type = Array.Find<Type>(_compiled.GetTypes(), delegate(Type t) { return t.Name == typeName; });	
			if (type == null)
				throw new ArgumentException(String.Format("Unable to locate type {0} in the compiled assembly.", typeName), typeName);
